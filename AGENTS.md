...
- 実装はゴールに向けて真っすぐな構造にし、根本原因を解決する。
- 条件分岐を追加する場合は、仕様上の理由をテストまたはコメントで明確にする。
- 目的が明確で、複雑さを下げる抽象化は許可する。
- linter / formatter は Biome に統一し、Biome のデフォルト設定を基本とする。
- Biome が対応しない、または不十分なファイル種別のために別の linter / formatter を追加する場合は、事前にユーザー確認を取る。
